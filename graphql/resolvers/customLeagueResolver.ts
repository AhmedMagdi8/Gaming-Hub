  ...
  Mutation: {
    ...
    addSpectatorToLeague: async (
      ...
      try {
        ...
        throw new GenericError(`Failed to add spectator: ${error.message}`, 500);
      }
    },
    ...
    generateMatches: async (
      ...
      try {
        ...
        const round = Math.log2(league.maxSeats);
        ...
        for (let i = 0; i < teams.length; i += 2) {
          const match = new Match({
            round,
            stage,
            participants: [{ team: teams[i] }, { team: teams[i + 1] }],
          });
        ...
        throw new GenericError(`Failed to generate matches: ${error.message}`, 500);
      }
    },
