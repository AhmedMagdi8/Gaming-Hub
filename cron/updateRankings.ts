  ...
  try {
    ...
    const usersByLeagueType = leagueNames.reduce((acc, type) => {
      acc[type] = users.filter(user => 
        user.league && 
        user.league.id && 
        user.league.id.name === type // Compare league name
      );
      return acc;
    }, {});
    ...
    for (const leagueName of leagueNames) {
      ...
      leagueUsers.sort((a, b) => b.league.currentMonthPoints - a.league.currentMonthPoints);
      ...
      leagueUsers.sort((a, b) => b.league.lastMonthPoints - a.league.lastMonthPoints);
      leagueUsers.forEach((user, index) => {
